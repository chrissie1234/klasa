...
module.exports = class extends Language {
	...
	constructor(...args) {
		...
		this.language = {
			...
			COMMAND_UNLOAD_DESCRIPTION: 'Unloads the BloxTech piece.',
			...
			COMMAND_RELOAD_DESCRIPTION: 'Reloads a BloxTech piece, or all pieces of a BloxTech store.',
			...
			COMMAND_INVITE: () => [
				...
				'This framework is made and maintained by BloxTech. The bot can be private, on choice of the customer that purchased this bot.'
			],
			COMMAND_INVITE_DESCRIPTION: 'Displays the invite link of the bot, to invite it to your guild.',
			COMMAND_INFO: [
				"The BloxTech Bot Framework is used for customized bots paid by the customer. BloxTech is hosting the bots 24/7 on their servers, like this bot.",
				'The BloxTech Bot Framework is ready to develop on. That\'s why it goes so fast!',
				'',
				'Some features of BloxTech Bot Framework include:',
				'• 🐇💨 Fast loading times with ES2017 support (`async`/`await`)',
				'• 🎚🎛 Per-bot/guild/user settings that can be configured by the customer of this bot',
				'• 💬 Command system with automated parameter resolving and the ability to load/reload commands on-the-fly',
				...
				'• ➕ "Extendables", which makes a new requested feature easy to implement',
				'• ⏲ "Tasks", which can be scheduled to run in the future, optionally repeating (for temporary punishments, repeated messages, etc.)',
				'',
				'We hope that we can cater to all audiences for customized bot orders. We do frequent updates and bugfixes when available, all included in your purchase.',
				"If you're interested in purchasing a bot from BloxTech, feel free to join our Discord server: https://discord.gg/MWWTa27"
			],
